...
void TGame::Move()
{
    ...
    if (tick >= 5)
    {
        if (!figure.Move(0, 1)) // íîâàÿ ôèãóðà
        {
            figure.Put(field.fld);
            figure.Shape(shpArr[rand() % shpArrCnt]);
            figure.Pos(fld_width / 2 - shp_width / 2, 0);
            if (figure.Check() > 0) // íîâàÿ èãðà
                field.Clear();
        }
        field.Burning();
        tick = 0;
...
bool TFigure::Move(int dx, int dy)
{
    ...
    if (chk >= 1)
    {
        Pos(oldX, oldY);
        if (chk == 2)
            return false; // íàäî ïðèçåìëèòü îáúåêò è ñîçäàòü íîâûé
    }
    return true;
}
...
{
    CalcCoord();
    for (int i = 0; i < coordCnt; i++) // ñíà÷àëà íàäî âåðíóòü ôèãóðó îáðàòíî
        if (coord[i].X < 0 || coord[i].X >= fld_width)
            return 1;
    for (int i = 0; i < coordCnt; i++) // åñëè îíà â ïîëå, è óïàëà, òî ïðèçåìëÿåì
